...
setup_instance() {
    ...
    if [ -f "$LEGACY/secrets.txt" ]; then cp "$LEGACY/secrets.txt" "$FOLDER/"; fi
    if [ -f "$LEGACY/access_token.txt" ]; then cp "$LEGACY/access_token.txt" "$FOLDER/"; fi

    # 3. Create Surgical config.py for this instance
    cat > config.py << EOF
