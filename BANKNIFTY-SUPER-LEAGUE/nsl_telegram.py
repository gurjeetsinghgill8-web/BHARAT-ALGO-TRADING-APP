"""
nsl_telegram.py — BANK NIFTY SUPER LEAGUE (BNSL) | LEGO 8: Telegram
====================================================================
All Telegram messaging for the BNSL engine.
Pre-defined templates for every event type.

BNSL-specific: lot size=30, monthly expiry, ₹180-₹250 premium range.
"""
...
def msg_startup(st_period: int, st_multiplier: float, lots: int) -> str:
    return (
        f"🏦 <b>BANK NIFTY SUPER LEAGUE v1.0 — STARTED</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"📐 SuperTrend : {st_period}/{st_multiplier} (5-min candles)\n"
        f"📦 Lots       : {lots} lot × 30 = {lots * 30} units\n"
        f"📅 Expiry     : Monthly (last Tuesday of month)\n"
        f"💰 Premium    : ₹180 – ₹250 range\n"
...
def msg_heartbeat(ltp: float, st_value: float, st_direction: str,
    ...
    now_ist   = datetime.now(ist).strftime("%d-%b %I:%M:%S %p")
    ...
    # LTP vs ST comparison — instant visual check
    diff     = ltp - st_value
    diff_str = f"+{diff:.2f} (LTP above ST ↑)" if diff > 0 else f"{diff:.2f} (LTP below ST ↓)"
    return (
        f"📊 <b>BANK NIFTY SUPER LEAGUE — PULSE</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"🕐 Time         : {now_ist}\n"
        f"BankNifty LTP : ₹{ltp:,.2f}  [LIVE]\n"
        f"SuperTrend    : ₹{st_value:,.2f} {dir_arrow}\n"
        f"LTP vs ST     : {diff_str}\n"
