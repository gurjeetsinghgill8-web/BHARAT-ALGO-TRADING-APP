"""
nsl_engine.py — BANK NIFTY SUPER LEAGUE (BNSL) | LEGO 7: Main Engine Loop
=========================================================================
STRICT STATE MACHINE — SuperTrend Edition v1.0
...
RULES (non-negotiable):
  ...
  9. Market: 9:16 AM start | 3:00 PM force close (all intraday)
 10. On non-new-candle cycles: refresh BOTH BankNifty LTP and option LTP
 11. Expiry: MONTHLY only — last Tuesday of month (Bank Nifty, no weekly)
"""
...
def _check_vps_only():
    ...
    if my_ip != VPS_IP:
        ...
        print()
        print("  BNSL engine MUST run on the VPS only.")
        print("  Run it at: ssh root@46.224.133.16")
        print("  Dashboard: http://46.224.133.16:8504")
        print("=" * 62)
        sys.exit(1)
    print(f"[VPS GUARD] IP confirmed: {my_ip} — BNSL OK to trade.")
...
def main():
    ...
    print("=" * 62)
    print("  BANK NIFTY SUPER LEAGUE  v1.0  —  SUPERTREND EDITION  ")
    print("=" * 62)
    ...
    print("  Rule 8: MONTHLY expiry only — last Tuesday of month")
    print("=" * 62)
