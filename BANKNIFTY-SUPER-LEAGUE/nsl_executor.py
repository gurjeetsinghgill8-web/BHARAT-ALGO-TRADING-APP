"""
nsl_executor.py — BANK NIFTY SUPER LEAGUE (BNSL) | LEGO 6: Order Engine
========================================================================
Handles all Upstox order placement for Bank Nifty options.

RULES:
  • Always BUYING options (CE for CALL, PE for PUT)
  • Strike: premium range ₹180–₹250, pick HIGHEST LTP in range
  • Expiry: MONTHLY ONLY — last Tuesday of current month
            (Bank Nifty weekly discontinued Nov 20, 2024 by SEBI)
...
# ─────────────────────────────────────────────────────────────
# Expiry calculation — MONTHLY (Bank Nifty only)
# ─────────────────────────────────────────────────────────────
def _last_tuesday_of_month(year: int, month: int) -> date:
    """
    Returns the last Tuesday of the given month.
    Bank Nifty monthly expiry = last Tuesday (NSE standardized).
    If that Tuesday is a holiday, Upstox API will have the adjusted date.
    """
    import calendar
...
def get_candidate_expiries() -> list[str]:
    """
    Bank Nifty MONTHLY expiry logic.
    Primary: fetches from Upstox option/contract API (most accurate — handles holidays).
    Fallback: calculates last Tuesday of current month, then next month.

    Returns up to 2 monthly expiry candidates (current + next month).
    Bank Nifty has NO weekly expiry (discontinued Nov 20, 2024).
    """
    try:
        token = db.get("upstox_access_token", "")
        resp  = requests.get(
        ...
        if resp.status_code == 200:
            ...
            today    = date.today()
            # Bank Nifty: keep only monthly expiries (last Tues of month)
            # Filter: only dates that are Tuesdays (weekday=1)
            monthly = [
                exp for exp in expiries
                if date.fromisoformat(exp) > today
                and date.fromisoformat(exp).weekday() == 1  # Tuesday
            ][:2]
            if monthly:
                utils.log(f"BNSL monthly expiries from API: {monthly}", "INFO")
                return monthly
        utils.log(f"Expiry API failed or no Tuesday expiries — using fallback", "WAIT")
    except Exception as e:
        utils.log(f"Expiry API exception: {e} — using fallback", "WAIT")

    # Fallback: compute last Tuesday of current and next month
    today = date.today()
    out   = []
    for delta_months in range(3):
        ...
        if len(out) >= 2:
            break
    utils.log(f"BNSL fallback monthly expiries (last Tue): {out}", "WAIT")
    return out


def get_next_expiry() -> str:
    """Convenience wrapper — returns the first candidate monthly expiry."""
...
# ─────────────────────────────────────────────────────────────
# Strike picker (₹180–₹250, highest LTP) — Bank Nifty
# ─────────────────────────────────────────────────────────────
